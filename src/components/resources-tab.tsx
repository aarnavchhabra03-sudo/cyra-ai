...
export interface LearningResourceRow {
  ...
  lesson_id: string;
  title: string;
  resource_type: string; // 'article' | 'documentation' | 'textbook' | 'reference' | 'video' | 'practice'
  url: string;
  source?: string;
  description?: string;
  duration?: string;
...
export default function ResourcesTab({ 
  ...
  // Session cache for fetched resource rows per lesson UUID
  ...
  const handleDummyGenerateClick = () => {
    setInfoNotice('AI Resource Pack Generation will be enabled in the next stage.');
    ...
    <div className="grid grid-cols-1 md:grid-cols-12 gap-6 h-full items-start">
        ...
        <div className="p-4 rounded-xl glass-panel border border-zinc-900">
          ...
          <div className="space-y-1">
            {nodeList.map((node) => {
              ...
              const lessonResourceCount = (resourcesCache[node.id] || []).length;
      ...
      <div className="md:col-span-9 p-6 md:p-8 rounded-2xl glass-panel border border-zinc-800/80 min-h-[500px] flex flex-col justify-between shadow-lg">
        {fetching ? (
          ...
          <div className="space-y-8">
            ...
            <div className="flex justify-between items-center border-b border-zinc-900 pb-4">
              ...
              <span className="text-[9px] font-mono text-zinc-400 bg-zinc-900 px-3 py-1.5 rounded-lg border border-zinc-800">
                {currentResources.length} {currentResources.length === 1 ? 'RESOURCE' : 'RESOURCES'}
              </span>
            </div>
          ...
          /* Empty State — No Resources Found in DB for Selected Lesson */
          <div className="py-16 text-center space-y-6 max-w-lg mx-auto my-auto">
            ...
            {infoNotice && (
              <div className="p-3.5 rounded-xl bg-indigo-500/10 border border-indigo-500/20 text-xs text-indigo-300 flex items-center justify-center gap-2 animate-fade-in">
            ...
            <button
              onClick={handleDummyGenerateClick}
              className="px-6 py-3 rounded-xl bg-gradient-to-r from-indigo-600 to-cyan-500 hover:opacity-90 text-white font-bold text-xs flex items-center justify-center gap-2 mx-auto transition-all shadow-lg shadow-indigo-500/20"
            >
